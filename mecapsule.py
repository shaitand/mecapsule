...
parser.add_argument(
    "--config", default=DEFAULT_CONFIG_FILE, help="Path to config.ini file"
)
parser.add_argument("--irc_server", help="IRC server to connect to")
parser.add_argument("--channel", help="IRC channel to join")
parser.add_argument(
    "--mode", choices=["bot", "client"], help="Mode to run: bot or client"
)
...
parser.add_argument(
    "--help",
    action="store_true",
    help="Show configuration environment variables and INI options",
)
...
debug = args.debug or os.getenv(
    "DEBUG", config["DEFAULT"].get("debug", "false")
).lower() in ["1", "true", "yes"]
...
async def bot_mode():
    ...
    await asyncio.gather(asyncio.to_thread(bot.run, forever=False), update_nodelist())
